...
const findESBuildForProject = async () => {
...
const writeOutDirPackageJson = async (esm: boolean) => {
  ...
  try {
    const userPackageJson = await readJson(PathManager.shard.packageJsonPath);
...
export const runESBuildForMainProcess: MainCommand = async (
  ...
  const esbuildConfigExtra = await loadESBuildConfigFromFile(esbuildConfigFile);
